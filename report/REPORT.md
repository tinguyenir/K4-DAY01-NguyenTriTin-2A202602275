...
Tiêu chí chọn lớp chính: object chiếm diện tích lớn nhất, object trung tâm, object theo mục tiêu nghiệp vụ hay ngữ cảnh toàn ảnh.
Khi nhiều chủ thể quan trọng tương đương thì ưu tiên lớp nào.
Khi ảnh không có một chủ thể trội thì có dùng lớp `other/background/uncertain` hay không.
Khi nào annotator phải escalation thay vì tự chọn theo cảm tính.
Có được suy đoán object bị che khuất hoặc không nhìn rõ hay không.
...
Chỉ cần nhìn thấy bao nhiêu phần trăm object thì vẫn gán nhãn?
Box bao phần visible hay ước lượng toàn bộ object (amodal)?
Có cần thuộc tính `occluded` hoặc `truncated` không?
Object rất nhỏ hoặc chỉ hiện một phần có được giữ không?
Nếu class không chắc chắn thì chọn `unknown`, bỏ qua hay escalation?
...
Hai object cùng lớp chạm nhau có tách thành hai instance không?
Biên giữa hai object gần cùng màu được xác định bằng tín hiệu nào?
Vùng bị object khác che có được suy đoán hay chỉ mask phần nhìn thấy?
Object quá nhỏ có cần polygon hay được bỏ theo ngưỡng kích thước?
Khi không xác định được biên chính xác thì annotator được phép dùng mức xấp xỉ nào?
Khi nào bắt buộc chuyển reviewer/lead quyết định?
...
Ảnh thô: dữ liệu đầu vào chưa có nhãn.
Guideline: tài liệu quy định taxonomy, phạm vi object, cách vẽ box/mask và cách xử lý trường hợp mơ hồ.
Ground truth: nhãn do con người tạo theo guideline; đây là dữ liệu chuẩn dùng cho training/evaluation.
Huấn luyện: model học từ dữ liệu đã được gán nhãn.
Prediction: đầu ra của model trên ảnh mới; có thể đúng hoặc sai.
QC/rework: reviewer kiểm tra tính đúng/nhất quán; annotation sai hoặc chưa rõ được trả lại để sửa.
4.2. Bảng tổng hợp
...
dừng xử lý;
không upload/commit/chia sẻ thêm;
giữ nguyên evidence cần thiết nhưng không sao chép dữ liệu sang nơi khác;
báo cho Lab Coach/mentor phụ trách để xác nhận cách xử lý;
chỉ tiếp tục khi nhận được hướng dẫn phù hợp.
---
